...
Cechy *ptFunboy = &tFunboy[0];		//przypisanie wskaźnikowi wartosci
...
void f_PodajDane ()
{
	...
	{	cin.clear();
		...
		cin>>v_name;
		cout<<"imie :"<<v_name<<endl;
	    int n = v_name.length();
	    char v_char_name[n + 1];
    
		strcpy(v_char_name, v_name.c_str()); 
		cout<<"imie :"<<v_name<<endl;
		v_name=f_PolskieZnaki(v_char_name);
		cout<<"imie :"<<v_name<<endl;
	...
	ptFunboy[0]=oFunboy;		//przypisanie wskaźnikowi wartosci
	...
	cout << "W dniu Twoich urodzin była następująca faza księżyca: " << oFunboy.faza_ksiezyca << endl; 
...
{	
	...
	//pFunboy = &oFunboy;			//przypisanie wskaźnikowi wartosci
